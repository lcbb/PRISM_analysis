...
% Created 10 November 2009 by Amanda Ng
    ...
    if nargin>0
        ...
        for n=1:2:length(args)
            ...
            switch lower(parameter)
                ...
                case 'scalelength_um'
                    if ~isnumeric(value)
                        error 'SCALELENGTH_um must be a numeric value'
                    end
                    scalelength_um = value;
    ...
    if scalelength==0
        if exist('scalelength_um', 'var')&& exist('pixsize', 'var')
            scalelength = scalelength_um/pixsize ;
        else
...
%     if scalelength_um <1    
%         text(mean(linepos(:,1)),linepos(1,2),0,[num2str(scalelength_um*1000) ' nm'],'verticalalignment',textalignment{1},'horizontalalignment',textalignment{2}, 'color', colour, 'fontweight', fontweight, 'parent', hg, 'fontsize',fontsize);
%     else
%         text(mean(linepos(:,1)),linepos(1,2),0,[num2str(scalelength_um) ' \mum'],'verticalalignment',textalignment{1},'horizontalalignment',textalignment{2}, 'color', colour, 'fontweight', fontweight, 'parent', hg, 'fontsize',fontsize);
%     end
