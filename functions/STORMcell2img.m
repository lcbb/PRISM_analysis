...
%% Main Function
%--------------------------------------------------------------------------
% Ic = zeros(h,w,Zs*length(active_channels)+numOverlays,'like',I(:,:,1)); 
if Zs > 1;   
    n=0;  
...
%             Ic(:,:,n) =  imadjust(I{n},[cmin(n),cmax(n)],[0,1]);
              I(:,:,n) =  imadjust(I(:,:,n),[cmin(n),cmax(n)],[0,1]);
           end
    end
end
if ~isempty(active_overlays)
    for n=active_overlays  % add overlays, if they exist
        I(:,:,numChns+n) = imadjust(overlays{n},[omin(n),omax(n)]);
    end
end


if nargout ~= 0
    Io = Ncolor(I,cMap); % Actually builds the RGB picture
else
     Ncolor(I,cMap); % Actually builds the RGB picture
end
